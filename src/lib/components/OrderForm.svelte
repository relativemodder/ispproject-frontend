...
<div class="max-w-md mx-auto p-4 border rounded shadow mb-4">
  ...
  <form on:submit|preventDefault={handleSubmit} class="space-y-4">
    <div>
      ...
      <input id="address" type="text" bind:value={address} required class="w-full border rounded px-3 py-2" />
    </div>
    <div>
      <label for="account_number" class="block mb-1">Номер абонента</label>
      <input id="account_number" type="text" bind:value={account_number} required class="w-full border rounded px-3 py-2" />
    </div>
    <div>
      <label for="contact_details" class="block mb-1">Контактные данные</label>
      <input id="contact_details" type="text" bind:value={contact_details} required class="w-full border rounded px-3 py-2" />
    </div>
    <div>
